...
class scELMoModel(nn.Module):
    ...
    def __init__(
        ...
        if gene_embeddings is not None:
            ...
            if emb_tensor.ndim != 2:
                raise ValueError(
                    f"gene_embeddings must be 2D, got shape {emb_tensor.shape}"
                )
    ...
    def load_weights(self, path: str, strict: bool = True):
        ...
        if not isinstance(data, dict):
            raise ValueError(
                f"Invalid pickle format. Expected dict, got {type(data)}."
            )
        ...
        if 'gene_list' in data:
            ...
            if loaded_genes != self.config.gene_names:
                mismatches = [
                    (i, l, c) for i, (l, c) in enumerate(zip(loaded_genes, self.config.gene_names))
                    if l != c
                ]
                if mismatches:
                    msg = (
    ...
    def get_coverage(self, gene_list: List[str]) -> Dict[str, Union[float, int, List[str]]]:
        ...
        unknown_genes = [
            gene for gene, idx in zip(gene_list, indices)
            if idx < 0
        ]
