...
class LocalDotProduct(torch.autograd.Function):
    ...
    dot = {"cpu": local_dot_product_cpu, "cuda": local_dot_product_cuda}
    dot_backward = {"cpu": local_dot_backward_cpu, "cuda": local_dot_backward_cuda}
    ...
    def forward(ctx, queries, keys, attn_mask, key_lengths, local_context):
        ...
        return LocalDotProduct.dot[queries.device.type](
            queries, keys, attn_mask, key_lengths, local_context
        )
    ...
    def backward(ctx, grad_input):
        ...
        grads = LocalDotProduct.dot_backward[queries.device.type](
            queries, keys, key_lengths, grad_input, local_context
        )
...
class LocalWeightedAverage(torch.autograd.Function):
    ...
    avg = {"cpu": local_weighted_average_cpu, "cuda": local_weighted_average_cuda}
    avg_backward = {
    ...
    def backward(ctx, grad_input):
        ...
        return LocalWeightedAverage.avg_backward[A.device.type](A, V, grad_input)
