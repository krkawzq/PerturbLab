...
class TransformerEncoderLayer(Module):
    ...
    def __init__(
        self, attention, d_model, d_ff=None, dropout=0.1, activation="relu", event_dispatcher=""
    ):
    ...
    def forward(self, x, attn_mask=None, length_mask=None):
        ...
        x = x + self.dropout(
            self.attention(
                x, x, x, attn_mask=attn_mask, query_lengths=length_mask, key_lengths=length_mask
            )
        )
...
class TransformerDecoderLayer(Module):
    ...
    def forward(
        self, x, memory, x_mask=None, x_length_mask=None, memory_mask=None, memory_length_mask=None
    ):
        ...
        memory_length_mask = memory_length_mask or LengthMask(
            x.new_full((N,), L_prime, dtype=torch.int64)
        )

        # First apply the self attention and add it to the input
        x = x + self.dropout(
            self.self_attention(
                x, x, x, attn_mask=x_mask, query_lengths=x_length_mask, key_lengths=x_length_mask
            )
        )
        x = self.norm1(x)
