...
class SparseDotProduct(torch.autograd.Function):
    ...
    dot = {"cpu": sparse_dot_product_cpu, "cuda": sparse_dot_product_cuda}
    dot_backward = {"cpu": sparse_dot_backward_cpu, "cuda": sparse_dot_backward_cuda}
    ...
    def backward(ctx, grad_output):
        ...
        SparseDotProduct.dot_backward[Q.device.type](Q, K, topk, grad_output, grad_Q, grad_K)
...
class SparseWeightedAverage(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        SparseWeightedAverage.avg_backward[weights.device.type](
            weights, values, topk, grad_output, grad_weights, grad_values
        )
    ...
    def backward(ctx, grad_output):
        ...
        if device.type == "cpu":
            ClusteredSparseDotProduct.dot_backward[Q.device.type](
                Q, K, groups, topk, grad_output, grad_Q, grad_K
            )
    ...
    def forward(ctx, weights, values, topk, groups, counts):
        ...
        if device.type == "cpu":
            ...
            ClusteredSparseWeightedAverage.avg[weights.device.type](
                weights, values, groups, topk, output
            )
        else:
            ...
            with torch.no_grad():
                ...
                indx_maps = torch.ones(
                    (block_counts.sum(), 4), device=weights.device, dtype=torch.int32
                )
                counts_cumsum = counts.cumsum(-1).int()
                total_blocks = block_counts.sum().item()
    ...
    def backward(ctx, grad_output):
        ...
        if device.type == "cpu":
            ClusteredSparseWeightedAverage.avg_backward[weights.device.type](
                weights, values, groups, topk, grad_output, grad_weights, grad_values
            )
        else:
