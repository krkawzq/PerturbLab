...
class PytorchAttentionWeightsRule(MappingRule):
    ...
    def matches(self, key):
        return self.weight_pattern in key or self.bias_pattern in key
    ...
    def apply(self, key, value):
        ...
        if self.weight_pattern in key:
            return [
                (
                    key.replace(self.weight_pattern, "attention.query_projection.weight"),
                    value[: N // 3],
                ),
                (
                    key.replace(self.weight_pattern, "attention.key_projection.weight"),
                    value[N // 3 : 2 * N // 3],
                ),
                (
                    key.replace(self.weight_pattern, "attention.value_projection.weight"),
                    value[2 * N // 3 :],
                ),
            ]
        if self.bias_pattern in key:
            return [
                (
                    key.replace(self.bias_pattern, "attention.query_projection.bias"),
                    value[: N // 3],
                ),
                (
                    key.replace(self.bias_pattern, "attention.key_projection.bias"),
                    value[N // 3 : 2 * N // 3],
                ),
                (
                    key.replace(self.bias_pattern, "attention.value_projection.bias"),
                    value[2 * N // 3 :],
                ),
            ]
...
class SimpleMapper:
    ...
    @classmethod
    def load_file(cls, filepath, model_root=None, map_location=None, **other_args):
    ...
    def __init__(self):
        super(PytorchMapper, self).__init__(
            [
                ...
                NotRule(OrRule(RegexRule(r"\.softmax_temp$", r""))),
            ],
            add_identity=False,
        )
...
class HugginfaceBertEncoderMapper(SimpleMapper):
    ...
    RULES = [
        ...
        RegexRule(r"layer\.(\d+)\.attention\.output\.dense", r"layers.\1.attention.out_projection"),
        RegexRule(r"layer\.(\d+)\.attention\.output\.LayerNorm", r"layers.\1.norm1"),
        RegexRule(r"layer\.(\d+)\.intermediate\.dense", r"layers.\1.linear1"),
        RegexRule(r"layer\.(\d+)\.output\.dense", r"layers.\1.linear2"),
        RegexRule(r"layer\.(\d+)\.output\.LayerNorm", r"layers.\1.norm2"),
    ]
...
class LongformerMapper(SimpleMapper):
    ...
    def __init__(self):
        super(LongformerMapper, self).__init__(
            ...
            + [NotRule(RegexRule(r"layer\.(\d+)\.attention\.self\.(query|key|value)_global", ""))],
            add_identity=False,
        )
