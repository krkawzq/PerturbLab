...
class ImprovedClusteredAttention(Module):
    ...
    def _topk_attention(
        self, Q, K, V, clusters, counts, topk, topk_values, A_bottomk, softmax_temp, query_lengths
    ):
        ...
        QK = clustered_sparse_dot_product(
            Q, K, topk, clusters, counts, query_lengths._lengths.int()
        )
        ...
        A_bottomk = clustered_broadcast(
            A_bottomk.unsqueeze(3), clusters, counts, torch.ones_like(counts, dtype=torch.float32)
        )
        A = A * (1.0 - A_bottomk)
        A = self.dropout(A)
    ...
    def forward(self, queries, keys, values, attn_mask, query_lengths, key_lengths):
        ...
        assert attn_mask.all_ones, (
            "Improved-clustered attention cannot " "use an arbitrary attention mask."
        )
        ...
        V_bottomk, A_bottomk = self._bottomk_attention(
            QK, values, clusters, counts, query_lengths, topk, softmax_temp
        )
