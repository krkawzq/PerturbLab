...
def GI_subgroup(pert_metric, GIs=None):
    """
    Calculate metrics for different GI (Genetic Interaction) subgroups.
    
    Args:
        pert_metric: Dictionary mapping perturbation names to their metrics
        GIs: Dictionary mapping GI types to lists of perturbation names. 
             If None, returns empty dictionary.
    
    Returns:
        Dictionary mapping GI types to their aggregated metrics
    """
    if GIs is None:
        return {}
...
def get_test_set_results_seen2(res, sel_GI_type, GIs=None):
    """
    Get relevant test set results for a specific GI type.
    
    Args:
        res: Dictionary containing test results
        sel_GI_type: Selected GI type to filter by
        GIs: Dictionary mapping GI types to lists of perturbation names.
             If None, returns empty results.
    
    Returns:
        Dictionary with filtered test results
    """
    if GIs is None or sel_GI_type not in GIs:
        return {}
