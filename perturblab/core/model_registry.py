...
from typing import Any, Callable, Type, TypeVar, Dict, Optional, Iterator
    ...
    **Design Philosophy**:
        ...
        def _register_wrapper(cls: Type[T]) -> Type[T]:
            ...
            if key in self._obj_map:
                if not force:
                    ...
                    logger.warning(
                        f"Overwriting model '{key}' in registry '{self._name}'"
                    )
    ...
    def add_child(self, registry: ModelRegistry, name: Optional[str] = None) -> None:
        ...
        if child_name in self._child_registries:
            logger.warning(
                f"Overwriting child registry '{child_name}' in '{self._name}'"
            )
    ...
    def get(self, key: str, default: Optional[Type[Any]] = None) -> Type[Any] | ModelRegistry | None:
    ...
    def build(self, key: str, *args, **kwargs) -> Any:
        ...
        **kwargs
        ...
        try:
            ...
            logger.error(
                f"Failed to build model '{key}' from registry '{self._name}': {e}"
            )
            raise
