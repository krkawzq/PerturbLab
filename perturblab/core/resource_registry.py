...
class ResourceRegistry:
        ...
        if duplicates:
            ...
            for dup_key, dup_resources in duplicates.items():
                if self.duplicate_policy == "error":
                    raise ValueError(
                        f"Duplicate resource key '{dup_key}' in registry '{self.key}'"
                    )
                elif self.duplicate_policy == "merge":
                    # Create nested registry for duplicates
                    self._resources_dict[dup_key] = self._merge_duplicates(
                        dup_key, dup_resources
                    )
                elif self.duplicate_policy == "drop":
                    # Keep first one (already in dict), drop the rest
                    logger.debug(f"Dropping {len(dup_resources) - 1} duplicate(s) for key '{dup_key}'")
    ...
    def _merge_duplicates(
        key: str,
        resources: list[Resource | ResourceRegistry]
    ) -> ResourceRegistry:
    ...
    def get(
        self,
        key: str,
        default: Resource | ResourceRegistry | None = None
