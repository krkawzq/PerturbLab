...
def _select_backend():
    ...
    from ..backends.python._ttest import log_fold_change_numpy, ttest_numpy
    ...
    logger.warning(
        "T-test backend: Python (NumPy/SciPy) - slower, consider installing compiled backends"
    )
...
def ttest(
    ...
    *,
    ...
    if method == "student":
        ...
        return _welch_ttest_impl(X.data, X.indices, X.indptr, group_id, n_targets, threads=threads)
...
def log_fold_change(
    ...
    *,
    ...
    return _log_fold_change_impl(
        X.data, X.indices, X.indptr, group_id, n_targets, threads=threads, pseudocount=pseudocount
    )
