...
class GeneGraph:
    ...
    def neighbors(self, node: int | str) -> Union[np.ndarray, List[str]]:
    ...
    def get_weights(self, node: int | str) -> np.ndarray:
        if isinstance(node, str):
            ...
            if node is None: return np.array([])
    ...
    def __repr__(self) -> str:
        return (
            f"GeneGraph(n_genes={self.n_nodes}, "
            f"n_edges={self.graph.n_unique_edges})"
        )
