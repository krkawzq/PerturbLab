...
class _VirtualViewBackedArray:
    ...
    def __getitem__(self, key):
        ...
        Args:
            key: Index or slice (supports int, slice, list, numpy array)

        Returns:
            Array data with virtual genes filled
        ...
        # Validate indices before normalization
        self._validate_index(row_idx, axis=0)
        self._validate_index(col_idx, axis=1)
        ...
        for i, target_idx in enumerate(target_col_indices):
            if target_idx >= len(self.gene_indices):
                raise IndexError(
                    f"Column index {target_idx} out of bounds for virtual array with "
                    f"{len(self.gene_indices)} genes"
                )
    ...
    def _validate_index(self, idx, axis: int):
        ...
        if isinstance(idx, int):
            if idx < -max_len or idx >= max_len:
                raise IndexError(
                    f"Index {idx} is out of bounds for axis {axis} with size {max_len}"
                )
        elif isinstance(idx, (list, np.ndarray)):
            idx_arr = np.asarray(idx)
            if len(idx_arr) > 0:
                min_idx, max_idx = idx_arr.min(), idx_arr.max()
                if min_idx < -max_len or max_idx >= max_len:
                    raise IndexError(
                        f"Index array out of bounds for axis {axis} with size {max_len}: "
                        f"got range [{min_idx}, {max_idx}]"
                    )
        # slice objects are handled by Python's slice.indices()
    ...
    def _normalize_index(self, idx, axis: int):
        ...
        Returns:
            Normalized index (slice or array)
        """
        max_len = self.shape[axis]
    ...
    def toarray(self):
        ...
        Warning:
            This loads the entire matrix into memory. For large backed datasets,
            this may cause memory issues.

        Returns:
            numpy.ndarray: Full materialized array
        """
        return self[:, :]
...
class CellData:
    ...
    def align_genes(
        ...
        **Zero-Copy Design**:
        ...
        if len(target_genes) == 0:
            raise ValueError("target_genes cannot be empty")

        if not target_genes.is_unique:
            duplicates = target_genes[target_genes.duplicated()].unique()
            raise ValueError(f"target_genes contains duplicates: {duplicates.tolist()}")

        # Materialize if already a virtual view
        if self._is_virtual_view:
            logger.debug(
                f"Materializing existing virtual view before realignment "
                f"({len(self._virtual_genes)} virtual genes)"
            )
        ...
        if self.gene_name_col:
            gene_to_idx = pd.Series(
                np.arange(len(self.adata.var_names)), index=self.adata.var[self.gene_name_col]
            )
        else:
            gene_to_idx = pd.Series(
                np.arange(len(self.adata.var_names)), index=self.adata.var_names
            )

        gene_indices = np.array([gene_to_idx.get(gene, -1) for gene in target_genes])

        # 4. No missing genes → AnnData view
        if len(missing_genes) == 0:
            actual_indices = gene_indices  # All should be >= 0
            if np.any(actual_indices < 0):
                raise ValueError(
                    "Internal error: gene_indices contains negative values when no genes are missing"
                )
        ...
        logger.debug(
            f"Creating virtual view with {len(missing_genes)} virtual genes "
    ...
    def split(
        ...
        Args:
            test_size: Fraction for test set (0 < test_size < 1)
        ...
        return {"train": self[train_idx], "test": self[test_idx]}
    ...
    def sample(
        ...
        Args:
            ...
            frac: Fraction of cells to sample (0 < frac <= 1)
        ...
        Raises:
            ValueError: If both n and frac are specified, or neither is specified
        ...
        if balance and n is None:
            raise ValueError("balance=True requires n to be specified")
    ...
    def to_tensor(
        ...
        if not sparse:
            if scipy.sparse.issparse(target):
                ...
                and hasattr(target, "__array__")
                and not isinstance(target, np.ndarray)
            ):
    ...
    def materialize(self) -> "CellData":
        ...
        # Use factory method to support subclasses
        return self._create_materialized_instance(new_adata)

    def _create_materialized_instance(self, new_adata: ad.AnnData) -> "CellData":
        """Create materialized instance from AnnData.

        This method can be overridden by subclasses to preserve their specific attributes.

        Args:
            new_adata: Materialized AnnData object

        Returns:
            Materialized CellData instance
        """
        instance = CellData(
            new_adata,
    ...
    def _create_from_existing(self, new_adata: ad.AnnData) -> "CellData":
        """Create new dataset from existing adata (clears virtual view).

        Optimized version that bypasses __init__ validation for views.

        Note:
            Subclasses should override this method to preserve their specific attributes.
        """
        # Use type(self) to support subclasses
    ...
    def _create_virtual_view(
        ...
        Args:
            ...
            gene_indices: Mapping to source genes (-1 for virtual)
            virtual_genes: Virtual gene fill values
            adata: Optional AnnData to use (defaults to self.adata)
        ...
        # Use type(self) to support subclasses
        instance = type(self)._initialize()
        instance.adata = adata if adata is not None else self.adata
        instance.cell_type_col = self.cell_type_col
        instance.gene_name_col = self.gene_name_col
        instance.cell_id_col = self.cell_id_col
        instance._duplicated_policy = self._duplicated_policy
    ...
    def _process_duplicates(self, adata: ad.AnnData, policy: DuplicatedGenePolicy) -> ad.AnnData:
    ...
    def _materialize_virtual_X(self, base_X):
        ...
        Args:
            base_X: Base expression matrix

        Returns:
            Materialized array with virtual genes filled

        Raises:
            ValueError: If gene indices are invalid
        """
        n_cells = self.adata.n_obs
        ...
        max_source_idx = (
            self._gene_indices[self._gene_indices >= 0].max()
            if np.any(self._gene_indices >= 0)
            else -1
        )
        if max_source_idx >= base_X.shape[1]:
            raise ValueError(
                f"Invalid gene index {max_source_idx} exceeds source matrix columns {base_X.shape[1]}"
            )
        ...
        if is_sparse:
            ...
            result = scipy.sparse.coo_matrix(
                (data, (rows, cols)), shape=(n_cells, n_target_genes), dtype=dtype
            ).tocsr()
        else:
