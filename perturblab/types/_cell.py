...
from perturblab.tools import split_cells, sample_cells_by_group, sample_cells_simple, sample_cells_weighted
logger = get_logger()
...
class CellData:
        ...
        **Zero-Copy Design**: 
        ...
        if len(missing_genes) == 0:
            ...
            if np.any(actual_indices < 0):
                raise ValueError("Internal error: gene_indices contains negative values when no genes are missing")
    ...
    def _materialize_virtual_X(self, base_X):
        ...
        max_source_idx = self._gene_indices[self._gene_indices >= 0].max() if np.any(self._gene_indices >= 0) else -1
