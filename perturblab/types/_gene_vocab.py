...
class GeneVocab(Vocab):
    ...
    def __init__(self, genes: list[str], default_token: str = "<unk>", default_index: int = -1):
    ...
    def _handle_duplicate_genes(
        ...
        if duplicate_policy == "error":
            if len(genes) != len(set(genes)):
                duplicates = [gene for gene in set(genes) if genes.count(gene) > 1]
    ...
    def select_genes(
        self, genes_to_keep: set[str] | list[str], keep_order: bool = True
    ) -> "GeneVocab":
