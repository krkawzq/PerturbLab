            ...
            try:
                ...
                if is_directory:
                    if not tmp_path.exists() or not tmp_path.is_dir():
                        raise ValueError(f"Creation function failed to create directory for key: {key}")
                    ...
                    logger.debug(f"Directory contains {num_files} files, total size: {file_size / 1024 / 1024:.1f} MB")
                else:
            ...
            def wrapper() -> Path:
                ...
                def create_fn(target_path: Path) -> None:
                    ...
                    if not target_path.exists():
                        raise ValueError(
                            f"Function did not create resource at: {target_path}"
                        )
                    ...
                    if is_directory:
                        ...
                        if not any(target_path.iterdir()):
                            raise ValueError(
                                f"Function created empty directory: {target_path}"
                            )
                    else:
                        ...
                        if target_path.stat().st_size == 0:
                            raise ValueError(
                                f"Function created empty file: {target_path}"
                            )
