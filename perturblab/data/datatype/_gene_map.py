...
class GeneMap:
    ...
    def _validate_mappings(self) -> None:
        ...
        if not self.symbol_to_id.is_function():
            raise ValueError(
                "symbol_to_id must be a function (each symbol maps to exactly one ID)"
            )
        
        if not self.symbol_to_id.is_injective():
            raise ValueError(
                "symbol_to_id must be injective (each symbol maps to unique ID)"
            )
    ...
    def standardize(self, alias: str) -> str | None:
        ...
        symbol_indices = self.alias_to_symbol.get_neighbors(
            alias_idx, check_bounds=False
        )
    ...
    def id(self, alias: str) -> str | None:
        ...
        if self.build_fast_mapping:
            ...
            alias_to_id_graph = self._build_alias_to_id_graph()
            id_indices = alias_to_id_graph.get_neighbors(
                alias_idx, check_bounds=False
            )
        else:
            ...
            id_indices = self.symbol_to_id.get_neighbors(
                symbol_idx, check_bounds=False
            )
    ...
    def query(
        ...
        if query_type == 'alias':
            ...
            raise ValueError(f"Invalid query_type: {query_type}. Must be 'alias', 'id', or 'symbol'.")
    ...
    def get_coverage_stats(self) -> dict:
        ...
        return {
            ...
            'avg_aliases_per_symbol': len(self.alias_vocab) / len(self.symbol_vocab)
                if len(self.symbol_vocab) > 0 else 0,
        }
    ...
    def load(cls, directory: str, build_fast_mapping: bool = True) -> 'GeneMap':
        ...
        # Load graphs
        alias_to_symbol = BipartiteGraph.load(
            os.path.join(directory, 'alias_to_symbol.npz')
        )
        symbol_to_id = BipartiteGraph.load(
            os.path.join(directory, 'symbol_to_id.npz')
        )
