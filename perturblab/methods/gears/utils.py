...
from perturblab.types import WeightedGraph, GeneVocab
...
def weighted_graph_to_dataframe(
    ...
    if include_node_names and graph.node_names is not None:
        ...
        df['source'] = df['source'].astype(int).map(
            {i: name for i, name in enumerate(graph.node_names)}
        )
        df['target'] = df['target'].astype(int).map(
            {i: name for i, name in enumerate(graph.node_names)}
        )
    else:
...
def build_perturbation_graph(
    ...
    **Graph Properties**:
    ...
    genes_set = set(genes)
    gene2go = {gene: go_terms for gene, go_terms in gene2go_all.items() 
    ...
    if genes_not_in_go:
        logger.warning(
            f"⚠️  {len(genes_not_in_go)} genes not in GO database (will be excluded):"
        )
