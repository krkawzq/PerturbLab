...
def extract_genes(
    ...
    set()
    """
    return set(g.strip() for g in perturbation.split("+") if g.strip() not in control_labels)
...
def split_perturbations_simulation(
    ...
    train_singles = [s for s in singles if extract_genes(s, control_labels).issubset(train_genes)]
    test_singles = [s for s in singles if extract_genes(s, control_labels).issubset(test_genes)]
    ...
    if split_type == "simulation_single":
        ...
        seen2_combos = [
            c for c in combos if len(extract_genes(c, control_labels) & train_genes) >= 2
        ]
...
def split_perturbations_combo_seen(
    ...
    all_genes = list(set().union(*[extract_genes(p, control_labels) for p in perturbations]))
    np.random.shuffle(all_genes)
    ...
    train_perts = [s for s in singles if extract_genes(s, control_labels).issubset(seen_genes)]
